...
elements = importmsh("./msh/patchtest_"*string(ndiv)*".msh",config)
nₚ = getnₚ(elements["Ω"])
...
# d = k\f
x = getfield(elements["Ω"][1].𝓒[1],:data)[:x][2]
y = getfield(elements["Ω"][1].𝓒[1],:data)[:y][2]
d = [w(x[i],y[i]) for i in 1:nₚ]
err = k*d - f
# f = check∇𝝭(elements["Γ₁"])
