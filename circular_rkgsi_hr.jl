...
𝒑 = "cubic"
# 𝒑 = "quartic"
config = YAML.load_file("./yml/circular_rkgsi_hr_"*𝒑*".yml")

ndiv = 8
elements, nodes = importmsh("./msh/circular_"*string(ndiv)*".msh", config)
end
...
# w₂₂₂₂(x,y) = 81n*(n-1)*(n-2)*(n-3)*(1+2x+3y)^abs(n-4)
 ...
 D = 1.0
 ν = 0.3
 w(x,y) = (a^2-x^2-y^2)/64*((5+ν)/(1+ν)*a^2-x^2-y^2)
w₁(x,y) = (-2*x)/64*((5+ν)/(1+ν)*a^2-x^2-y^2)+(a^2-x^2-y^2)/64*(-2*x)
w₂(x,y) = (-2*y)/64*((5+ν)/(1+ν)*a^2-x^2-y^2)+(a^2-x^2-y^2)/64*(-2*y)
w₁₁(x,y) = (-2)/64*((5+ν)/(1+ν)*a^2-x^2-y^2)+2*(-2*x)/64*(-2*x)+(a^2-x^2-y^2)/64*(-2)
w₂₂(x,y) = (-2)/64*((5+ν)/(1+ν)*a^2-x^2-y^2)+2*(-2*y)/64*(-2*y)+(a^2-x^2-y^2)/64*(-2)
w₁₂(x,y) = (-2*x)/64*(-2*y)+(-2*y)/64*(-2x)
w₁₁₁(x,y) = (24*x)/64
w₁₁₂(x,y) = (8*y)/64
w₁₂₂(x,y) = (8*x)/64
w₂₂₂(x,y) = (24*y)/64
